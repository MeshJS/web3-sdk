import { Bech32mTokenIdentifier, SparkWallet, getNetworkFromSparkAddress } from "@buildonspark/spark-sdk";
...
export class EmbeddedSparkWallet {
    ...
    async getWalletInfo(): Promise<Spark.WalletInfo> {
        const [sparkAddress, staticDepositAddress, identityPublicKey] = await Promise.all([
    ...
    async getBalance(): Promise<{
        ...
        if (balanceResult.tokenBalances && Array.isArray(balanceResult.tokenBalances)) {
    ...
    async signMessageWithIdentityKey(
        ...
        const result = await this.sparkWallet.signMessageWithIdentityKey(messageString, compact);
